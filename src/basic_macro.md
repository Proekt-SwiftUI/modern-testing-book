...
Применение атрибута `@Suite` является необязательным, но с помощью атрибута можно настроить внешний вид тестов, применить различные кортежи`⁑` и другое.
...
> `⁑` Раннее я не упомнял о концепции кортежей для макросов `@Test` и `@Suite`, но чуть позже ты познакомишься с этим понятием.
